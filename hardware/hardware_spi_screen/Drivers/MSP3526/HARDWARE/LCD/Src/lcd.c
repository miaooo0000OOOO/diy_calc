...
void LCD_WR_REG(u8 data)
{
	LCD_CS_CLR;
	LCD_RS_CLR;
	// printf("LCD_CS:%d", HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_11));
	// printf("LCD_RS:%d", HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_10));
...
void LCD_Init(void)
{
	SPI2_Init(); //硬件SPI2初始化 // CubeMX生成
	LCD_GPIOInit();//LCD GPIO初始化 // CubeMX生成
	LCD_RESET(); // LCD 复位
	//*************3.5 ST7796S IPS初始化**********//
	LCD_WR_REG(0x11);
