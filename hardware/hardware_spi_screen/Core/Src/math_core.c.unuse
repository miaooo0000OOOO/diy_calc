...
AST_Node *recu_calc(AST_Node *node)
{
    ...
    {
        node->token->value = get_var_value((char *)node->token->value)->value;
...
float get_delta(AST_Node *node, float x)
{
    assign_real_var("x", x);
    float *fl, *fr;
    Token *tl, *tr;
    tl = calc(node->left);
    tr = calc(node->right);
    cast_int2float_inplace(tl);
    cast_int2float_inplace(tr);
    fl = (float *)&tl->value;
    fr = (float *)&tr->value;
    return *fl - *fr;
}
...
AST_Node *solve(AST_Node *node)
{
    ...
    {
        cop = deep_copy_node(node);
        y = get_delta(cop, x - EPSILON);
        recu_free_ast(cop);
        cop = deep_copy_node(node);
